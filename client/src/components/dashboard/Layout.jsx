...
const Layout = ({ children, userType }) => {
  ...
  // Check if current path is a classroom path
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      {/* Sidebar - Hidden in classroom views */}
      {!isClassroomView && (
      ...
      {/* Main Content - Full width in classroom views */}
      <div className={`flex-1 ${isClassroomView ? 'p-0' : 'p-8'}`}>
        {children}
      </div>
    </div>
  );
